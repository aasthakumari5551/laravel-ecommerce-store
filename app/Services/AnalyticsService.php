...
class AnalyticsService
{
    ...
    public function overview(int $days = 30): array
    {
        ...
        $current = $this->periodStats($from, now());
        ...
        return [
            ...
            'aov'           => $current['aov'],           // average order value
            'customers'     => $current['customers'],
            'revenue_delta' => $this->delta($prior['revenue'],   $current['revenue']),
            'orders_delta'  => $this->delta($prior['orders'],    $current['orders']),
            'aov_delta'     => $this->delta($prior['aov'],       $current['aov']),
            'customer_delta'=> $this->delta($prior['customers'], $current['customers']),
        ];
    }
     ...
     */
    public function topProducts(int $limit = 10, int $days = 30): \Illuminate\Support\Collection
    {
        return DB::table('order_items')
                 ->join('orders', 'orders.id', '=', 'order_items.order_id')
                 ->join('products', 'products.id', '=', 'order_items.product_id')
                 ->where('orders.payment_status', PaymentStatus::Paid->value)
                 ->where('orders.paid_at', '>=', now()->subDays($days))
                 ->selectRaw('
                     products.id,
                     products.name,
                     products.slug,
    ...
    public function topCategories(int $days = 30): \Illuminate\Support\Collection
    {
        return DB::table('order_items')
                 ->join('orders', 'orders.id', '=', 'order_items.order_id')
                 ->join('products', 'products.id', '=', 'order_items.product_id')
    ...
    public function orderStatusBreakdown(): array
    {
        ...
        return collect(OrderStatus::cases())->map(fn ($s) => [
            'status' => $s->label(),
    ...
    public function lowStockProducts(int $limit = 15): \Illuminate\Support\Collection
    {
    ...
    public function newCustomersByDay(int $days = 30): array
    {
    ...
    public function couponPerformance(int $limit = 10): \Illuminate\Support\Collection
    {
        return DB::table('coupon_usages')
                 ...
                 ')
                 ->groupBy('coupons.id', 'coupons.code', 'coupons.discount_type')
    ...
    {
        $result = Order::where('payment_status', PaymentStatus::Paid->value)
    ...
    /** Returns signed percentage change between two values. */
